...
seu <- qs::qread(fs::path("gdm/results/", "gdm_subtyped.qs"))$treg@meta.data
unique(seu$cluster)
seu %>%
...
ma <- readr::read_csv(fs::path("gdm/results/", "top_subtyping_de_markers.csv")) %>%
...
pp <- qs::qread(fs::path("gdm/results/", "propeller.qs"))
tp <- pp[[1]]
cp <- pp[[2]]
