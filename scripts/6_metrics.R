...
unique(seu$cluster)
seu %>% dplyr::select(multi_q, cond_cln) %>% unique() %>% dplyr::count(cond_cln)
# CONTROL: 13 ; GDM: 10 # 
# Note in metadata, GDM is 9 as one sample is missing blood pressure.

# Mean and SD of cells per patient?
seu %>% dplyr::group_by(multi_q, cond_cln) %>% dplyr::transmute(ncells=n()) %>% unique() %>%
    ungroup() %>% dplyr::group_by(cond_cln) %>%
    dplyr::transmute(meanpp=mean(ncells), sdpp=sd(ncells)) %>%
    unique()

# Total number of cells per group?
seu %>% dplyr::select(cond_cln) %>% dplyr::count(cond_cln)
...
show_top_marks <- function(x, i, l='treg', n=3){
  x %>% dplyr::filter(ident==i & label==l) %>% dplyr::top_n(n, abs(cal)) %>% dplyr::arrange(-abs(cal)) %>%
    dplyr::mutate(gsign = paste0(gene, ifelse(sign(cal)==1, "+", "-"))) %>% pull(gsign) %>% paste0(collapse=", ")
}
for(e in seq(0,11)){cat(paste0(show_top_marks(ma, e), "\n"))}
for(e in seq(0,10)){cat(paste0(show_top_marks(ma, e, 'cd4', 4), "\n"))}
...
pp = qs::qread(fs::path("gdm/results_v2/", 'propeller.qs'))
tp = pp[[1]]; cp=pp[[2]]
...
dedata[[1]]$data %>% dplyr::group_by(label, cluster) %>% dplyr::summarise(sigc = sum(is_sig, na.rm=T), ng=n())

# Mean sig DE genes per cluster
dedata[[1]]$data %>% dplyr::group_by(label, cluster) %>% dplyr::summarise(sigc = sum(is_sig, na.rm=T), ng=n()) %>%
    dplyr::mutate(isnai=stringr::str_detect(cluster, 'Naive')) %>% dplyr::group_by(isnai) %>%
...
bulkp %>% pull(model_name) %>% unique() %>% length() # N genes returned

top_markers = readr::read_csv(fs::path(outdir, "top_subtyping_de_markers.csv"))

top_markers %>% dplyr::filter(gene=="TXNIP") %>% dplyr::select(-contains('CASE'))
