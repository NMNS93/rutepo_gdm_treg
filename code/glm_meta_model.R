...
medication_reformat <- function(meta) {
    ...
    aspirin <- as.numeric(meta$low_dose_aspirin) %>%
        ifelse(is.na(.), 0, .) %>%
        as.factor()
...
getclusterprop <- function(x) {
    x@meta.data %>%
        dplyr::group_by(multi_q, seurat_clusters, cluster) %>%
        dplyr::count() %>%
        dplyr::ungroup() %>%
...
plot_meta_lr <- function(meta, feat, extra, linecol = "red") {
    ...
    ggplot(meta, aes(x = !!sym(feat), y = control_b)) +
        ...
        geom_point(data = extra, color = "grey") +
        theme_light() +
...
recurrent_deg_filter <- function(x) {
    x %>%
        dplyr::filter(is_sig & abs(avg_log2FC) > 0.1) %>%
        dplyr::select(seurat_clusters, cluster, gene) %>%
        dplyr::group_by(gene) %>%
        dplyr::count() %>%
        dplyr::filter(n >= 3) %>%
        pull(gene)
...
cluster_deg_filter <- function(x, y) {
    x %>%
        dplyr::filter(is_sig & abs(avg_log2FC) > 0.1) %>%
        dplyr::filter(stringr::str_detect(cluster, y)) %>%
        pull(gene) %>%
        unique()
}
...
get_coexp_genes <- function(coexp) {
    ...
    ma_genes <- dplyr::filter(ma, module == "Naive-Activation-21") %>%
        arrange(-abs(`kME_Naive-Activation-21`)) %>%
        head(10) %>%
        pull(gene_name)
    return(ma_genes)
}
...
model_fold_with_metrics <- function(fi) {
    ...
    fold_metrics <- tibble(
        fold_no, fold_id,
        model = list(mod), model_aic = mod_aic, x_truth = list(x_truth), x_prob = list(x_prob),
...
build_roc_data <- function(strims_top_mods, cd4_mod, cd4s_mod, strims_mod) {
    cd4_roc <- cd4_mod %>%
        ...
        dplyr::select(dataset, model_num, model_name_cd4 = model_name, fold_no, fold_id, all_y_auc, y_prob, y_truth) %>%
        left_join(
    ...
    cd4s_roc <- cd4s_mod %>%
        ...
        dplyr::select(dataset, model_num, model_name_strims = model_name, fold_no, fold_id, all_y_auc, y_prob, y_truth) %>%
        left_join(
            strims_top_mods %>% select(model_name_strims = model_name, model_name_cd4)
        )

    strims_roc <- strims_mod %>%
        left_join(
            strims_top_mods %>% select(-all_y_auc),
            by = "model_name"
        ) %>%
...
roc_mini <- function(x, data_name) {
    x %>%
        ...
        dplyr::select(dataset, model_name, fold_no, fold_id, all_y_auc, all_y_f1, y_prob, y_truth)
}
