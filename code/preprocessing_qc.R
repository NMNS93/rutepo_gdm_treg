...
#' Run GDM pre-processing pipeline
...
#' Filter dataset for GDM or control patient samples
...
#' Run QC Pipeline using thresholds determined from prior exploratory analysis
...
RunSCTransformIntegration <- function(x) {
  ...
  anchors <- FindIntegrationAnchors(
    object.list = x, normalization.method = "SCT",
...
#' Integrate TENX data pipeline using SelectIntegrationFeatures
...
#' Add Metadata
AddRutepoMeta <- function(tenx, meta_csv, cols_regex = NULL, add_multiq = T) {
  if (add_multiq) {
    # Setup fields
...
#' Filter TCR genes to ensure that clusters are not driven by TCR status
...
#' Calculate summary statistic of median cell per patient
...
#' Calculate the average number of cells per patient per cluster
cells_per_pat_clust <- function(x, label) {
  dplyr::count(x@meta.data, multi_q, seurat_clusters) %>%
    group_by(seurat_clusters) %>%
    dplyr::summarise(avg = mean(n), sd = sd(n), med = median(n), iqr = stats::IQR(n)) %>%
    mutate(label = label)
}
