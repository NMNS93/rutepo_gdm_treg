# Run differential expression analysis within subtypes
...
#' Annotate clusters from CSV file
...
#' Select the top N differentially expressed genes
de_top_n_genes <- function(de, n) {
    dplyr::group_by(de, ident) %>%
        dplyr::top_n(n, abs(CONTROL_avg_log2FC)) %>%
        pull(gene)
}

#' Select the top markers
...
#' Arrange subtypes by expression values
